...
// === Using cho các thành phần gameplay ===
using CGP.Gameplay.Items;                 // ItemManager
...
using CGP.Gameplay.Systems;               // Player, CurrencyManager, ...
...
public class GameManager : MonoBehaviour
{
    ...
    // Không tham chiếu trực tiếp UI_Manager để tránh lỗi asmdef
    [SerializeField, Tooltip("Drag your UI_Manager here (any namespace).")]
    ...
    private void Awake()
    {
        ...
        if (uiManager == null) uiManager = FindUIManager();              // <— tìm theo tên lớp
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        if (uiManager == null) uiManager = FindUIManager();
    ...
    // ===== Helpers =====
    /// <summary>Tìm component có tên lớp "UI_Manager" ở bất kỳ namespace/assembly nào.</summary>
    private MonoBehaviour FindUIManager()
    {
        // Ưu tiên object đang active trong scene
        foreach (var mb in FindObjectsByType<MonoBehaviour>(FindObjectsInactive.Include, FindObjectsSortMode.None))
        {
            if (mb != null && mb.GetType().Name == "UI_Manager")
                return mb;
        }
        // Fallback: quét cả assets (ít khi cần)
